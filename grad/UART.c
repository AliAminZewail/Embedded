...
//COINFIGURE GPIO
   ...
   */
        (*((volatile uint32*)0x4000551C))=0x03;
        (*((volatile uint32*)0x40005420))=0x03;
        (*((volatile uint32*)0x4000552C))=0x3311;
...
//COINFIGURE GPIO
        ...
        GPIO_PortCPCTL = 0X11001111; //THE Fn. IS UART
        break;
...
//COINFIGURE GPIO
        ...
        GPIO_PortCPCTL = 0X00111111; //THE Fn. IS UART
        break;
...
void UART_Transmit(UART_N0_t uart, uint8 data)
{
    ...
    if (uart == UART0)
    {
        while (GET_BIT(UART0_FR,5) == 1); //wait utill buffer is not full
        UART0_DR = data;
    }
    else if (uart == UART1)
    {
